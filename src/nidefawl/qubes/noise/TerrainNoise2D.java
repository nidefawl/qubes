...
public class TerrainNoise2D extends AbstractNoiseGen {
    private final OpenSimplexNoise noise;
    private final double scaleX;
    private final double scaleZ;
    private final int nOctaves;

    public TerrainNoise2D(long seed, double scaleX, double scaleZ, int nOctaves) {
    ...
    public double get(int x, int z) {
        ...
        for (int n = 0; n < this.nOctaves; n++) {
            ...
            d += this.noise.eval(dX, dZ)*dAmplitude;
            dFreq *= 2.0D;
            dAmplitude /= 2.0D;
        }
        return d;
