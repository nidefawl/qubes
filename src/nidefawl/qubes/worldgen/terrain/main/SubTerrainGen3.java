...
import nidefawl.qubes.worldgen.terrain.main.SubTerrainGen3.NoiseData;

/**
 * @author Michael Hept 2015
 * Copyright: Michael Hept
 */
public class SubTerrainGen3 extends SubTerrainGen {
    /**
     * @author Michael Hept 2015
     * Copyright: Michael Hept
     */
    ...
    public SubTerrainGen3(TerrainGeneratorMain t) {
        this.main = t;
    ...
    public int generate(int cX, int cZ, int x, int y1, int y2, int z, HexBiome hex, SubTerrainData data, double[] d) {
        ...
        double dBaseHeight = (noise2D.get(cX|x, cZ|z))*4.8D;
...
//        System.out.println(outerScale);
        ...
        for (int y = y1; y < y2; y++) {
            double dYH = clamp10((y+0+8*flattenScale)/(double)df);
            double dyoff = 0.8D;
            if (dYH >= dyoff) {
