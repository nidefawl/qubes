...
public class PNGDecoder {
    ...
    private void copyGreyscaleToARGB(ByteBuffer buffer, byte[] curLine) {
    ...
    private void copyPALtoARGB(ByteBuffer buffer, byte[] curLine) {
        if (paletteA != null) {
            for (int i = 1, n = curLine.length; i < n; i += 1) {
                ...
                byte a = paletteA[idx];
                buffer.put(a).put(r).put(g).put(b);
            }
        } else {
            for (int i = 1, n = curLine.length; i < n; i += 1) {
                int idx = curLine[i] & 255;
    ...
    private void copyRGBtoARGB(ByteBuffer buffer, byte[] curLine) {
        if (transPixel != null) {
            ...
            for (int i = 1, n = curLine.length; i < n; i += 3) {
                buffer.put((byte) 0xFF).put(curLine[i]).put(curLine[i + 1]).put(curLine[i + 2]);
            }
        }
    }
