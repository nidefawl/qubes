...
public class TesselatorState {
    ...
    public void setClientStates(ByteBuffer buffer) {
        ...
        if (useNormalPtr) {
            ...
            if (Client.GL_ERROR_CHECKS) Engine.checkGLError("glNormalPointer");
            GL11.glEnableClientState(GL11.GL_NORMAL_ARRAY);
            offset+=1;
        }
        if (useTexturePtr) {
            GL11.glTexCoordPointer(2, GL11.GL_FLOAT, stride*4, (ByteBuffer) buffer.position(offset*4));
            if (Client.GL_ERROR_CHECKS) Engine.checkGLError("glTexCoordPointer");
...
//            offset+=2;
    ...
    public void setAttrPtr() {
        ...
        if (Client.GL_ERROR_CHECKS) Engine.checkGLError("AttribPtr "+0);
        
        int offset = 4;
        if (useNormalPtr) {
            GL20.glEnableVertexAttribArray(1);
            GL20.glVertexAttribPointer(1, 3, GL11.GL_BYTE, false, stride*4, offset*4);
            if (Client.GL_ERROR_CHECKS) Engine.checkGLError("AttribPtr "+1);
            offset+=1;
        }
        if (useTexturePtr) {
            GL20.glEnableVertexAttribArray(2);
            GL20.glVertexAttribPointer(2, 2, GL11.GL_FLOAT, false, stride*4, offset*4);
            if (Client.GL_ERROR_CHECKS) Engine.checkGLError("AttribPtr "+2);
            offset+=2;
        }
        if (useColorPtr) {
            GL20.glEnableVertexAttribArray(3);
            GL20.glVertexAttribPointer(3, 4, GL11.GL_UNSIGNED_BYTE, true, stride*4, offset*4);
            if (Client.GL_ERROR_CHECKS) Engine.checkGLError("AttribPtr "+3);
            offset+=1;
        }
        if (useTexturePtr2) {
            GL20.glEnableVertexAttribArray(4);
            GL20.glVertexAttribPointer(4, 2, GL11.GL_SHORT, false, stride*4, offset*4);
            if (Client.GL_ERROR_CHECKS) Engine.checkGLError("AttribPtr "+4);
            offset+=1;
        }
        if (useAttribPtr1) {
            GL20.glEnableVertexAttribArray(5);
            GL20.glVertexAttribPointer(5, 4, GL11.GL_SHORT, false, stride*4, offset*4);
            if (Client.GL_ERROR_CHECKS) Engine.checkGLError("AttribPtr "+5);
            offset+=2;
        }
    }
    public void drawVBO(int mode) {
        GL11.glDrawArrays(mode, 0, vertexcount);
        if (Client.GL_ERROR_CHECKS) Engine.checkGLError("glDrawArrays ("+vertexcount+", texture: "+useTexturePtr+")");
    }
