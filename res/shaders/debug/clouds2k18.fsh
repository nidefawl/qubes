...
#define illumstrength (1.85+sin(u_time*0.2)*0.35)

#define cld_march_steps (125)
#define cld_coverage (0.25+sin(u_time*1.3+4)*0.02)
#define cld_thick (122.0)
float cld_absorb_coeff = (1.*0.99);
#define cld_wind_dir vec3(0, 0, -u_time * .161)
#define cld_sun_dir normalize(vec3(0, 0.3/*abs(sin(u_time * .3))*/, -1))
...
float density_func(
	...
	p.x*=1.1;
	p.z*=0.9;
	p.xz *= 0.92+0.05*vec2(fbm_clouds(p * 2.032, 2.6434, .5, .5));
...
vec4 render_clouds(
	...
	float cutoff = dot(eye.direction, vec3(0, 0.9, 0));
	...
	for (int i = 0; i < steps; i++) {
		...
		float dens = density_func((cloud.pos)*1.2, cloud.height);
		// float dens1 = pow(dens, 4)*2.0;
		float dens1 = dens*smoothstep (cld_coverage, cld_coverage * .025, dens);
		float densb = density_func(cloud.pos-vec3(20.1), cloud.height);
		float dens2 = densb*smoothstep (cld_coverage, cld_coverage + .025, densb);
		dens = dens1*0.3+dens2*0.6;
		integrate_volume(
			cloud,
	...
	vec3 p = (cloud.origin * .001 + cld_wind_dir)*1;
	...
	vec3 ccol = cloud.C*v3;
	ccol = pow(ccol, vec3(0.97,0.9,0.8)*1.0)+pow(ccol, vec3(0.27,0.26,0.76)*0.9)*0.3*0.8;
	vec4 ret = vec4(ccol*0.7, calpha);
	// return vec4(vec3(0.5),0);
	return ret;
}
