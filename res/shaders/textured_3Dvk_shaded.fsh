...
#pragma define "SHADOW_MAP_RESOLUTION" "1024.0"
...
float lookupShadowMap(vec3 v, float factor) {
    ...
    for (int x = -SOFT_SHADOW_TAP_RANGE; x <= SOFT_SHADOW_TAP_RANGE; x++) {
        for (int y = -SOFT_SHADOW_TAP_RANGE; y <= SOFT_SHADOW_TAP_RANGE; y++) {
            ...
            if (SHADOW_COMPARE(v.z, textureLod(texShadow, v.xy+offs, 0.0).r)) {
                s += 1;
            }
        }
    }
...
float getShadow2() {
    ...
    vec4 v = getShadowTexcoord(in_matrix_shadow.shadow_split_mvp[0], inPosition);
    vec4 v2 = getShadowTexcoord(in_matrix_shadow.shadow_split_mvp[1], inPosition);
    vec4 v3 = getShadowTexcoord(in_matrix_shadow.shadow_split_mvp[2], inPosition);
