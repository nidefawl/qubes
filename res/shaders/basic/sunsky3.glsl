...
vec3 Kr1 = vec3(0.28867780436772762, 0.4978442963618773, 0.6616065586417131); // air
...
// uniform float rayleigh_collection_power, mie_collection_power, mie_distribution;
 float rayleigh_brightness = 1.8;
 float mie_brightness = 0.089;
...
vec3 asdf4r(vec3 eyedir, vec3 lightdir, float moon){
...
{
	// vec3 asdf = prop.position.xyz;
	// vec3 c_updir    = vec3(0.0, 1.0, 0.0);
 //    vec3 uu = normalize(cross(viewDir, c_updir));
 //    vec3 vv = normalize(cross(uu, viewDir));
	// vec3 res = compute_sky_light(vec3(0), viewDir, earth, sun);
	// return res*100;
	return asdf4r(viewDir, sunDirection, 0);
}

vec3 moonsky(vec3 viewDir, vec3 sunDirection)
{
	// vec3 asdf = prop.position.xyz;
	// vec3 c_updir    = vec3(0.0, 1.0, 0.0);
 //    vec3 uu = normalize(cross(viewDir, c_updir));
 //    vec3 vv = normalize(cross(uu, viewDir));
	// vec3 res = compute_sky_light(vec3(0), viewDir, earth, sun);
	// return res*100;
	return asdf4r(viewDir, sunDirection, 1);
}