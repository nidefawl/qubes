  	...
  	if (pos.y>0) {
		// out_Color = vec4(vec3(0), 1);
  		...
  		// 	out_Color = vec4(vec3(1, 0, 0), 0.5);
		    ...
		    float alpha = 0;
		    float sc = (1.0);
		    float ff = 1.0/PARALLAX_LAYERS;
		    for (int i = 0; i < PARALLAX_LAYERS; ++i)
		    {
		        // 3 parallax layers of clouds
		        vec2 uv2 = _uv;//vec2(_uv.x+aa*0.01,_uv.y-aa*0.01);
		       float h = fbm((uv2) * freq) * 0.72;
		        h += max(0, h-0.2)*fbm(vec2(t * 0.001, t * 0.002)*0.1 + _uv * freq * 0.3) * 0.74;
		        // h += min(max(0, h-0.1)*20, 1)*fbm(vec2(t * 0.0003, t * 0.0025)*0.1 + _uv * freq * 0.6) * 0.1;
		    ...
		    float scale = 1.0 - max(0, (length(uv)-10)/20.0);
		    // if (length(uv)<10.2)
	  			out_Color = vec4(vec3(0.9+max(-0.4, 1-alpha*1.4)*1.8), clamp(alpha*4, 0, scale));//vec4(vec3(0), clamp(a, 0, 1)*0.9);
  		}
  	}
